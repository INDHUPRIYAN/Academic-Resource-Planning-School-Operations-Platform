"""
Seed the "Avinashi Government Girls Higher Secondary School (Avinashi GGHSS)"
institution and generate its master timetable. This is the authoritative,
self-contained seed for the school: institution details, classes, sections,
teachers, subjects, allocations, class teachers, activities, fixed events,
scheduling rules AND teacher availability are all created here.
...
Institution type is "school"; every scheduling rule below is written into the
institution's configuration (not hardcoded in the engine), so the same solver
serves colleges too - see Sri_Shakthi_seed.py.

Usage:
    python -m uvicorn app.main:app --port 8010     # server must be running
    python -m app.seeds.Avinashi_GGHSS_seed        # (or: python seed_demo_school.py)
"""
import json
import os
import sys
...
# ---------------------------------------------------------------- source data
SCHOOL_NAME = "Avinashi Government Girls Higher Secondary School (Avinashi GGHSS)"
PPD, WD = 8, 5
FRIDAY = 4  # Monday = 0
...
def main():
    ...
    # Teacher availability - hard constraints applied BEFORE generation so the single
    # solve already honours them. Grids live in avinashi_availability.py.
    from app.seeds.avinashi_availability import BLOCKED, ALLOWED, grid_from_blocked, grid_from_allowed
    print("== Applying teacher availability ==")
    for name, spec in BLOCKED.items():
        api("PUT", f"/teachers/{teacher_id[name]}/availability", json=grid_from_blocked(spec))
    for name, spec in ALLOWED.items():
        api("PUT", f"/teachers/{teacher_id[name]}/availability", json=grid_from_allowed(spec))
    print(f"  availability grids set for {len(BLOCKED) + len(ALLOWED)} teachers\n")
    ...
    print(f"\nInstitution #{SID} — {SCHOOL_NAME}")
    print(f"Login: {ADMIN_EMAIL} / {ADMIN_PASSWORD}   Teachers: <name>@avi.gghss.in / {TEACHER_PASSWORD}")
